	...
	workspaceRoutes := router.DefineRoutes(
		...
		router.NewRoute(http.MethodGet, "/workspaces", a.listWorkspaces,
			...
			router.WithSwagger(
				...
				router.WithDescription(`Lists workspaces owned by the authenticated user, including unattached workspaces, filtered, sorted, and paginated. Returns the standard paginated list shape ({data, total}).`),
				router.WithTags("Workspaces"),
				router.WithSuccessResponse(http.StatusOK, "Workspaces", router.WithJSONContent(dto.WorkspaceListResponseResponse{})),
			),
		),
