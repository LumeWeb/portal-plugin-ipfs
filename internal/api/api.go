...
See also:.*`),
				...
				router.WithPathParam("name", "Record name", ""),
				router.WithPathParam("type", "Record type (A, AAAA, CNAME, TXT, etc.)", ""),
				router.WithRequestBody(&dto.RecordDeleteRequest{}, "Optional content selector: when set, only the record with this content is deleted; when absent or empty, the whole RRSet is deleted", false),
