...
const (
	...
	// ErrKeyWorkspaceNotFound is returned when a workspace does not exist or is
	// not owned by the requesting user (indistinguishable on purpose).
	ErrKeyWorkspaceNotFound core.ErrorType = "WORKSPACE_NOT_FOUND"
	// ErrKeyWorkspaceInvalidState is returned when a lifecycle transition is
	// requested from a status it does not allow (e.g. suspending a workspace
	// that is not ready, or deleting one already `deleting`).
	ErrKeyWorkspaceInvalidState core.ErrorType = "WORKSPACE_INVALID_STATE"
	// ErrKeyWorkspaceDisabled is returned when the workspace service is not
	// enabled/configured.
	ErrKeyWorkspaceDisabled core.ErrorType = "WORKSPACE_DISABLED"
	// ErrKeyWorkspaceDuplicate is returned when a website already has a live
	// workspace.
	ErrKeyWorkspaceDuplicate core.ErrorType = "WORKSPACE_DUPLICATE"
	// ErrKeyWorkspaceProviderUnavailable is returned when the Coolify provider
	// is not reachable/available.
	ErrKeyWorkspaceProviderUnavailable core.ErrorType = "WORKSPACE_PROVIDER_UNAVAILABLE"
	// ErrKeyWorkspaceOperationFailed is returned for a non-recoverable
	// suspend/resume/delete provider or database failure.
	ErrKeyWorkspaceOperationFailed core.ErrorType = "WORKSPACE_OPERATION_FAILED"
)
...
func init() {
	...
	core.MustRegisterDefaultErrorMessages(Namespace, map[core.ErrorType]core.ErrorDefinition{
		...
		ErrKeyWorkspaceNotFound:            {Key: ErrKeyWorkspaceNotFound, Message: "Workspace not found."},
		ErrKeyWorkspaceInvalidState:        {Key: ErrKeyWorkspaceInvalidState, Message: "Workspace is not in a valid state for this operation."},
		ErrKeyWorkspaceDisabled:            {Key: ErrKeyWorkspaceDisabled, Message: "The workspace service is disabled."},
		ErrKeyWorkspaceDuplicate:           {Key: ErrKeyWorkspaceDuplicate, Message: "This website already has a workspace."},
		ErrKeyWorkspaceProviderUnavailable: {Key: ErrKeyWorkspaceProviderUnavailable, Message: "The workspace provisioning provider is unavailable."},
		ErrKeyWorkspaceOperationFailed:     {Key: ErrKeyWorkspaceOperationFailed, Message: "Workspace operation failed."},
	})
	...
	core.MustRegisterErrorCodes(Namespace, map[core.ErrorType]int{
		...
		ErrKeyWorkspaceNotFound:            http.StatusNotFound,
		ErrKeyWorkspaceInvalidState:        http.StatusConflict,
		ErrKeyWorkspaceDisabled:            http.StatusServiceUnavailable,
		ErrKeyWorkspaceDuplicate:           http.StatusConflict,
		ErrKeyWorkspaceProviderUnavailable: http.StatusServiceUnavailable,
		ErrKeyWorkspaceOperationFailed:     http.StatusInternalServerError,
	})
}
